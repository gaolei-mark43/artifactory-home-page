<template>
    <div>
      ...
      <div class="inline-container">
        <p>目前共有{{ dataCount }}个本地库,包含{{ packageTypeCount.Maven }}个maven库、{{packageTypeCount.Npm}}个npm库、{{packageTypeCount.Conan}}个conan库、{{packageTypeCount.Conda}}个conda库、{{ packageTypeCount.CocoaPods }}个cocoapods库、{{ packageTypeCount.Pub }}个flutter/Dart库、{{ packageTypeCount.Pypi }}个pypi库</p>
        <p><br>描述中local file cache代表仓库已开启缓存功能，会对应缓存到私服本地，用以提速。</p>
        ...
        <table class="table" id="table">
            ...
            <tbody>
                <tr v-for="item in data" :key="item.key">
                    <td><a :href="'/remote-repo-detail/' + item.key">{{ item.key }}</a></td>
  ...
  export default {
    ...
    methods: {
      fetchData() {
        axios
          .get('http://http://127.0.0.1/:9098/remote_repo')
          .then((response) => {
